...
func (s *service) Create(ctx context.Context, info model.SightingInfo) (string, error) {
	...
	// !Важно, что до этого, у нас для всех примеров ручка Create, возвращала исключительно uuid. Чтобы не переделывать приходится лезть внизу ещё раз в базу, но при написании с самого начала, нужно бы делать так, чтобы возвращалось вся запись, чтобы её можно было использовать для кеширования
