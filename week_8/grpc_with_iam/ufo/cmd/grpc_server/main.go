...
import (
	...
	ufoV1 "github.com/olezhek28/microservices-course-examples/week_8/grpc_with_iam/shared/pkg/proto/ufo/v1"
)
...
func main() {
	...
	// Создаем и регистрируем health check сервис
	healthServer := health.NewServer()
	grpc_health_v1.RegisterHealthServer(s, healthServer)

	// Устанавливаем статус сервиса как SERVING
	healthServer.SetServingStatus("ufo.v1.UFOService", grpc_health_v1.HealthCheckResponse_SERVING)
	...
	go func() {
		log.Printf("🚀 UFO gRPC server listening on %s\n", grpcAddr)
		err = s.Serve(lis)
		if err != nil {
	...
	<-quit
	log.Println("🛑 Shutting down UFO gRPC server...")
	s.GracefulStop()
	log.Println("✅ UFO Server stopped")
}
